...
- The extension now also registers the four modifier types with ComposedDistributions' generated flat-vector codec (`register_leaf_wrapper!`, ComposedDistributions#189): `flat_dimension`/`unflatten`/`flatten`/`reconstruct` now correctly peel `Affine`/`Weighted`/`Transformed`/`Modified` (fixed structure hidden, a `thin`'s factor surfacing as a free parameter) rather than treating the wrapper as an opaque, un-peeled leaf. Previously only the runtime `params_table` walk saw through these wrappers; the generated codec silently fell back to an un-peeled (and therefore `flat_dimension`-undercounting) leaf until this registration was added.
