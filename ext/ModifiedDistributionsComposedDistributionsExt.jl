...
# `cumulative` / `transform`). Type owner: ComposedDistributions
# (`Sequential`). The extension depends on both, so there is no piracy.
module ModifiedDistributionsComposedDistributionsExt

import ModifiedDistributions: affine, weight, thin, cumulative, transform
...
transform(d::Sequential, op) = transform(observed_distribution(d), op)
